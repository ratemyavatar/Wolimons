...
echo.
echo     1  Sign in with a browser ON THIS MACHINE. setup.bat creates
echo        the tunnel and the DNS record for you.
echo.
echo     2  Paste a tunnel TOKEN from the Cloudflare app on your phone.
echo        You then add the hostname (the DNS record) in the app.
echo        Use this when there is no browser on this machine.
echo.
if not exist "%ENVFILE%" (
...
rem ===========================================================================
rem  Shared by both flows: ask which address the site will live at.
rem ===========================================================================
:tunnel_hostname
set "HOSTNAME="
...
rem ===========================================================================
rem  4a. Browser sign-in - creates the tunnel and the DNS record itself
rem ===========================================================================
:tunnel_browser
...
rem ===========================================================================
rem  4b. Tunnel token from the Cloudflare app - no browser on this machine
rem ===========================================================================
:tunnel_token
call :tunnel_hostname

rem --- Also useful: do they want the www hostname too? -------------------
set "ADDWWW="
...
echo.
echo   On your phone, in the Cloudflare app: open the tunnel
echo   "wolimons" - Add a replica - and copy the install command.
echo   The token is the long eyJ... part of it.
echo.
echo   Anyone with this token can attach to your tunnel - treat it
echo   like a password and don't share it.
echo.
set "TOKEN="
set /p "TOKEN=  Paste the eyJ... token here: "
if "!TOKEN!"=="" goto menu

rem --- A Cloudflare tunnel token is a JWT, which always starts eyJ. ------
echo !TOKEN!| findstr /r "^eyJ" >nul
if errorlevel 1 (
  echo.
  echo   That doesn't look like a Cloudflare token - it should start
  echo   with eyJ. Make sure you copied the whole eyJ... string.
  echo.
  pause
  goto menu
)
...
if errorlevel 1 (
  echo.
  echo   The tunnel service didn't start. Run this and check for errors:
  echo   "!CFEXE!" service install "!TOKEN!"
  echo   The token can be re-copied in the app: tunnel - Add a replica.
) else (
  echo   Running.
)
...
echo   DONE.
echo.
echo   Last step - on your phone, in the Cloudflare app: open the
echo   tunnel "wolimons" - Public Hostname - Add a public hostname:
echo.
echo     Subdomain:  blank for the root, or "www"
echo     Domain:     !HOSTNAME!  - choose your domain
echo     Service:    HTTP  localhost:!SITEPORT!
echo.
echo   Save it. That adds the DNS record for you automatically.
echo   DNS can take a minute - wait and retry before changing anything.
echo   ------------------------------------------------------------
echo.
echo   Nothing needs an open inbound port any more. Close the one
echo   setup opened?
